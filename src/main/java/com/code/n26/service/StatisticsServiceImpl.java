...
public class StatisticsServiceImpl implements StatisticsService {
	...
	public Statistics getStatisticsForLast60Seconds() {
		...
		if(!transactions.isEmpty()) {
			...
			DoubleSummaryStatistics stats = transactions.stream().filter(x -> !isTransactionOlderThanSixtySeconds(x, Instant.now()))
	...
	private boolean isTransactionOlderThanSixtySeconds(Transaction transaction, Instant now) {
